...
class MqttModule:
	...
	def on_device_manual_action(mosq, userdata, msg):
		...
		if ("m/" + config.misc["roomID"] + "/air-sensors-night-mode" == msg.topic):
			ValuesModule.set("air_sen_night_mode", (False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/air-sensors-oled-on" == msg.topic):
			ValuesModule.set("air_sen_oled", (False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/water-tester-night-mode" == msg.topic):
			ValuesModule.set("air_sen_night_mode", (False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/water-tester-oled-on" == msg.topic):
			ValuesModule.set("water_tester_oled", (False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/main-controller-night-mode" == msg.topic):
			ValuesModule.set("main_con_night_mode", ( False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/main-controller-oled-on" == msg.topic ):
			ValuesModule.set("con_lcd", ( False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/doser-one-night-mode" == msg.topic):
			ValuesModule.set("doser_one_night_mode", (False if value == "0" else True))
		if ("m/" + config.misc["roomID"] + "/doser-two-night-mode" == msg.topic):
			ValuesModule.set("doser_two_night_mode", (False if value == "0" else True))
	...
	def on_water_tester(mosq, userdata, msg):
		...
		try:
			...
			ValuesModule.set("sal", int(str(data["sal"]).replace('.', '')), False)
		except Exception as e:
			logging.exception(e)